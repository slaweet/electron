...
function spawnUpdate (args, detached, callback) {
  ...
  try {
    ...
    if (spawnedProcess && !isSameArgs(args)) {
      ...
      spawnedProcess = spawn(updateExe, args, { detached: detached })
      spawnedArgs = args || []
    }
  } catch (err) {
...
exports.checkForUpdate = (updateURL, callback) => {
  return spawnUpdate(['--checkForUpdate', updateURL], false, (error, stdout) => {
    ...
    try {
      ...
      if (json !== null) {
        const releasesToApply = json.releasesToApply
        if (releasesToApply !== null) {
          if (typeof releasesToApply.pop === 'function') {
            update = releasesToApply.pop()
          }
        }
      }
    } catch (jsonError) {
